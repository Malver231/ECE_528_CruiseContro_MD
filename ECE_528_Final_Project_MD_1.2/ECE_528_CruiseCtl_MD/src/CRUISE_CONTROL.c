...
#define P_GAIN 1.0f// Proportional gaIn
#define I_GAIN 2.5f //Integral gain
...
void CRUISE_CONTROL(uint16_t distance_mm)
{
    ...
    if(dist_mm<CRUISE_MIN_DIST)
    {
       ...
       // prev_duty=command_duty;
        SDK_STATE=AVOID;
        //try commenting this and update IDLE instead
        return;
    }
...
void AVOID_COMMAND(uint16_t distance_mm)
{
   ...
   case AVOID_IDLE:
        ...
        Motor_Left(1700, 1700);

        SDK_AVOID_STATE=CHECK_LEFT;
        LED2_Output(RGB_LED_WHITE); //Use for truble shooting
        break;
